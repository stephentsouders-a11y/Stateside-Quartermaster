...
const u=new URL(route,BASE);u.searchParams.set('preview_theme_id',THEME);let resp=null,error=null;const started=Date.now();try{resp=await p.goto(u.href,{waitUntil:'domcontentloaded',timeout:70000});await p.waitForLoadState('load',{timeout:16000}).catch(()=>{});await p.waitForTimeout(1300)}catch(e){error=String(e?.message||e)}
...
const row={key,label,route,httpStatus:resp?.status()??null,error,theme,triggerVisible,categories,categoryAudits,categoryCount:categories.length,subcategoryCount:categoryAudits.reduce((s,x)=>s+x.subcategories.length,0),typeCount:categoryAudits.reduce((s,x)=>s+x.typeSamples.reduce((q,t)=>q+t.types.length,0),0),invalidHrefs,errors:errors.slice(0,30),elapsedMs:Date.now()-started,...metrics,previewOk:theme.id===THEME};
...
if(row.httpStatus!==200||!row.previewOk||error||!triggerVisible||invalidHrefs.length||row.brokenImages)process.exitCode=2;await c.close();await b.close();