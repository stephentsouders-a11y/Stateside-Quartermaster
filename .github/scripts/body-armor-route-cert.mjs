...
const URL='https://www.statesideqm.com/collections/literature?preview_theme_id=158561894555';
const OUT='cert-literature';
const ROUTE='Literature';
const REQUIRED=['Author','Publisher','War/Conflict','Branch','Topic'];
...
async function inspect(page,mobile){
  ...
  let readySeen=false;
  try{
    await page.waitForFunction(()=>document.querySelector('#MainContent')?.dataset.sqRefineV3Ready==='true'||document.querySelector('#MainContent')?.dataset.sqRefineV3Error,{timeout:20000});
    readySeen=true;
  }catch{}
  await sleep(500);
  ...
  if(data.ready==='true'&&data.groups.length){
    ...
    const idx=await groups.evaluateAll(gs=>gs.findIndex(g=>!g.open));
    if(idx>=0){
      const d=groups.nth(idx),s=d.locator(':scope>summary');
      try{
...
console.log(JSON.stringify({route:ROUTE,pass:report.pass,desktop:report.desktop,mobile:report.mobile},null,2));
