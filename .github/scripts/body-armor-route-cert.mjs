// Knives re-cert after null-safe facet race + default-50 fast path.
import { chromium } from 'playwright';
const URL='https://www.statesideqm.com/collections/knives-axes-cutlery?preview_theme_id=158561894555';
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
async function gotoHealthy(p){let r=null;for(let i=0;i<3;i++){r=await p.goto(URL,{waitUntil:'domcontentloaded',timeout:45000});if((r?.status()||0)<500)break;await sleep(1500*(i+1));}return r;}
...
for(const [name,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
  ...
  const d=await p.evaluate((name)=>{
    ...
    const cards=results?[...results.querySelectorAll('.product-grid__item,li.grid__item,.product-card-wrapper,.card-wrapper,[data-product-card]')]:[];
    ...
    const catChoices=[...(sidebar?.querySelectorAll('input[type="checkbox"]')||[])].length;
    ...
    return {checks,error:main?.dataset.sqRefineV3Error||'',visibleCards:visibleCards.length,cols,catChoices,size:[...new Set(size)],sticky,isolation,legacy,oldSize,nativeColumn};
  },name);
