...
const sidebarApplicable=route!=='/collections/state-guard-naval-militia-themed-merchandise';const row={key,label,route,sidebarApplicable,httpStatus:resp?.status()??null,error,theme,triggerVisible,categories,categoryAudits,categoryCount:categories.length,subcategoryCount:categoryAudits.reduce((s,x)=>s+x.subcategories.length,0),typeCount:categoryAudits.reduce((s,x)=>s+x.typeSamples.reduce((q,t)=>q+t.types.length,0),0),invalidHrefs,errors:errors.slice(0,30),elapsedMs:Date.now()-started,...metrics,previewOk:theme.id===THEME};
