...
async function inspectChoice(groupTitle,label){
  ...
  const out={groupTitle,label,attrs:found.attrs,clickedChecked:found.checked,oldUrl,afterUrl:p.url(),changed:oldUrl!==p.url()||JSON.stringify(before)!==JSON.stringify(after),...result};await p.close();return out;
}
...
const checks=[];for(const g of inventory){for(const label of g.labels){checks.push(await inspectChoice(g.title,label));console.log('BODY_ARMOR_FILTER_CHECK '+JSON.stringify(checks.at(-1)));}}
console.log('BODY_ARMOR_FILTER_CHECKS '+JSON.stringify(checks));
const failures=[];for(const c of checks){if(c.missing)failures.push(`${c.groupTitle}: ${c.label}: missing ${c.reason}`);else if(!c.changed)failures.push(`${c.groupTitle}: ${c.label}: no URL/product change`);else if(!c.ready||c.error||c.cards<1)failures.push(`${c.groupTitle}: ${c.label}: ready=${c.ready} error=${c.error} cards=${c.cards}`)}
